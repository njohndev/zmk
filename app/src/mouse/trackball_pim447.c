...
static void thread_code(void *p1, void *p2, void *p3) {
    ...
    while (true) {
        ...
        if (pos_dx.val1 != 0 || pos_dy.val1 != 0) {
            zmk_hid_mouse_movement_update(convert_speed(pos_dx.val1), convert_speed(pos_dy.val1));
            send_report = true;
        }
...
int zmk_trackball_pim447_init() {
    k_thread_create(&thread, thread_stack, STACK_SIZE, thread_code, NULL, NULL, NULL,
