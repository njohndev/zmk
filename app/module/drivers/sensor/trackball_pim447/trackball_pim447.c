...
static int trackball_pim447_read_reg(const struct device *dev, uint8_t reg, uint8_t *value) {
    ...
    int status = i2c_reg_read_byte(data->i2c_dev, DT_INST_REG_ADDR(0), reg, value);
    if (status < 0) {
        LOG_ERR("Failed to read register 0x%02x: %d", reg, status);
        return status;
    }
...
static int trackball_pim447_read_axis(const struct device *dev, uint8_t reg_negative,
                                      uint8_t reg_positive, int32_t *value) {
...
static int trackball_pim447_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_POS_DX) {
        status = trackball_pim447_read_axis(dev, TRACKBALL_PIM447_REG_LEFT,
                                            TRACKBALL_PIM447_REG_RIGHT, &data->dx);
        if (status < 0) {
            return status;
        }
    }
    ...
    if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_POS_DY) {
        status = trackball_pim447_read_axis(dev, TRACKBALL_PIM447_REG_UP, TRACKBALL_PIM447_REG_DOWN,
                                            &data->dy);
        if (status < 0) {
            return status;
        }
    ...
    if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_POS_DZ) {
        uint8_t value;
        status = trackball_pim447_read_reg(dev, TRACKBALL_PIM447_REG_SWITCH, &value);
        if (status < 0) {
            return status;
        }
...
static int trackball_pim447_init(const struct device *dev) {
    ...
    data->i2c_dev = DEVICE_DT_GET(DT_NODELABEL(i2c1));
